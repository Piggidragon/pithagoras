...
/** The folder chats start in when no project is chosen. */
...
export type ProjectErrorCode = "invalid" | "exists" | "missing" | "protected";
...
/** The Home folder, made if it is not there yet. */
export function ensureHome(root: string): string {
  const home = path.join(root, HOME_NAME);
...
export function createProject(root: string, rawName: string, instructions?: string): ProjectInfo {
  ...
  if (name === HOME_NAME) {
    throw new ProjectError("invalid", `"${HOME_NAME}" is taken: it is where chats start by default`);
  }
  const target = path.join(root, name);
