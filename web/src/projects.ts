/** The folder name Home lives in; see server/src/projects.ts. */
export const HOME_NAME = "home";

/** What a chat's folder is called on screen: Home, or the project's name. */
export function projectLabel(workspace: string): string {
  const name = workspace.split("/").pop() || workspace;
  return name === HOME_NAME ? "Home" : name;
}

/** Mirrors the server's slugify so the preview matches the folder that gets made. */
export function slugify(input: string): string {
  return input
    .normalize("NFKD")
    .replace(/[̀-ͯ]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, "-")
    .replace(/-{2,}/g, "-")
    .replace(/^[-._]+|[-._]+$/g, "")
    .slice(0, 64);
}

export function bytesLabel(bytes: number): string {
  if (bytes < 1024) return `${bytes} B`;
  if (bytes < 1024 ** 2) return `${(bytes / 1024).toFixed(1)} KB`;
  if (bytes < 1024 ** 3) return `${(bytes / 1024 ** 2).toFixed(1)} MB`;
  return `${(bytes / 1024 ** 3).toFixed(1)} GB`;
}
