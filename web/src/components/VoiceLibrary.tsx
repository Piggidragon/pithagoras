...
export function VoiceLibrary({value,onChange,onError}:{value:string;onChange:(id:string)=>void;onError:(message:string)=>void}){
 ...
 return <div className="space-y-2">
  <div className="block text-xs text-fg-muted">Speaking voice<Select aria-label="Speaking voice" className="mt-1.5 w-full" value={value} onChange={onChange} options={[{value:'design',label:'Designed voice'},{value:'aria',label:'Aria',hint:'Reference clone'},...voices.map(v=>({value:v.id,label:v.name,text:v.name,hint:v.kind==='clone'?'Reference clone':'Designed'}))]}/></div>
  {selected&&<div className="rounded-lg border border-line p-3 space-y-2"><p className="text-xs text-fg-muted">{selected.instruction}</p>{selected.kind==='clone'&&<><audio aria-label="Voice reference preview" controls preload="none" className="w-full h-9" src={`/api/voice/presets/${selected.id}/audio`}/><p className="text-xs text-fg-faint">{selected.transcript}</p></>}<button type="button" disabled={busy} className="inline-flex items-center gap-1 text-xs text-danger" onClick={async()=>{if(!await confirmDialog({title:`Delete voice “${selected.name}”?`,confirmLabel:'Delete',danger:true,deletes:true}))return;setBusy(true);try{await request('/'+selected.id,'DELETE');setVoices(v=>v.filter(p=>p.id!==selected.id));onChange('design');window.dispatchEvent(new Event('voice-config-changed'));}catch(e){onError((e as Error).message);}finally{setBusy(false);}}}><LuTrash2/>Delete voice</button></div>}
  ...
  {adding&&<div className="rounded-lg border border-line bg-surface p-3 space-y-3">
   ...
   <div className="block text-xs">Voice type<Select aria-label="Voice type" className="mt-1.5 w-full" value={kind} onChange={v=>setKind(v as 'clone'|'design')} options={[{value:'clone',label:'Clone from a recording'},{value:'design',label:'Design from a description'}]}/></div>
